...
function DateRangeFields({
  ...
  const avail = useMessages().listing.availability;
  return (
    <div className="mx-auto w-full max-w-[260px]">
      <div className="grid grid-cols-2 gap-3">
        <div className="min-w-0">
          <p className="mb-1 text-center text-xs font-medium text-gray-600">{avail.start}</p>
          <input
            type="date"
            value={start}
        ...
        <div className="min-w-0">
          <p className="mb-1 text-center text-xs font-medium text-gray-600">{avail.end}</p>
          <input
            type="date"
            value={end}
...
function TimeRangeRow({
  ...
  const avail = useMessages().listing.availability;
  const fromLabel = avail.from;
  const toLabel = avail.to;
  return (
    <div>
      <p className="mb-2 text-sm font-medium text-gray-700">{label}</p>
      <div className="grid grid-cols-2 gap-3">
        <div className="min-w-0">
          <p className="mb-1 text-center text-xs text-gray-500">{fromLabel}</p>
          <input
            type="time"
            value={start}
            onChange={(event) => onStartChange(event.target.value)}
        ...
        <div className="min-w-0">
          <p className="mb-1 text-center text-xs text-gray-500">{toLabel}</p>
          <input
            type="time"
            value={end}
            onChange={(event) => onEndChange(event.target.value)}
...
export function Step5Availability({ draft, setDraft }: StepProps) {
  const t = useMessages();
  const avail = t.listing.availability;
  const dayOptions = [
    { id: "Mo" as const, label: avail.weekdayMo },
    { id: "Tu" as const, label: avail.weekdayTu },
    { id: "We" as const, label: avail.weekdayWe },
    { id: "Th" as const, label: avail.weekdayTh },
    { id: "Fr" as const, label: avail.weekdayFr },
    { id: "Sa" as const, label: avail.weekdaySa },
    { id: "Su" as const, label: avail.weekdaySu },
  ];
  ...
  return (
    ...
    >
      ...
      <section className="mb-8">
        <div className="mb-4 overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
          ...
          <div className="px-4 pb-4 text-center">
            <p className="font-semibold text-gray-900">{avail.regularHours}</p>
            <p className="mt-0.5 text-sm text-gray-500">
              {avail.regularHoursDesc}
            </p>
          </div>
        </div>
        <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
          <p className="mb-3 text-sm font-medium text-gray-700">{avail.daysOfWeek}</p>
      ...
      <section>
        <h3 className="text-base font-bold text-gray-900">{avail.optionalExceptions}</h3>
        <p className="mt-0.5 text-sm text-gray-500">
          {avail.optionalExceptionsDesc}
        </p>
        <div className="mb-6 mt-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="font-semibold text-gray-900">{avail.pauseListing}</p>
              <p className="text-sm text-gray-500">{avail.pauseDesc}</p>
            </div>
            <button
              type="button"
              role="switch"
        ...
        <div className="mb-6">
          <h4 className="text-sm font-bold text-gray-900">{avail.blockDates}</h4>
          <p className="mt-0.5 text-sm text-gray-500">{avail.blockDatesDesc}</p>
          {!showBlockPicker ? (
            <button
              type="button"
